...
const FormItem = Form.Item;
  ...
  class DiamondIncrease extends React.Component {
    ...
    getListFare = () => {
      ...
      app.$api.diamondListFare().then(res => {
        if (res.data.length != 0) {
          ...
          let arr = [{startWeight:'',endWeight:'',rule:'',level:4}];
          this.setState({
            ListFareData:arr,
            proLoading: false,
          })
    ...
    delPrice = (item,index) => {
      if (item.id == undefined || item.id == '') {
        let newData = [...this.state.ListFareData];
    ...
    newUpdataPrice = (item,index) => {
      ...
      let num1 =  this.props.form.getFieldValue(`startWeight_${index}`);
      let num2 =  this.props.form.getFieldValue(`endWeight_${index}`);
      let startWeight =  Number(num1);
      let endWeight =  Number(num2);
      let rule =  this.props.form.getFieldValue(`rule_${index}`);
  ...
  render() {
    ...
    const list = () => {
        let res = this.state.ListFareData.map((item,index) => {
          return(
      ...
      <div id={styles.diamondWrap} >
        ...
        <div className={styles.tabel}>
          <div className={styles.trWrap}>
              ...
              <div className={styles.thWrapC}></div>
          </div>
