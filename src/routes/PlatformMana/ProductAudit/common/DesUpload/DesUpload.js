...
class DesUpload extends React.Component {
  ...
  componentWillReceiveProps() {
    ...
    // console.log(this.props.result.proId);
  ...
  componentDidMount() {
    ...
    var referencePriceTF = false;
    this.props.result.proAllData.textures.forEach(ielem=>{
      if(ielem.referencePrice){
        referencePriceTF= true;
      }
    })
    this.props.result.proAllData.referencePriceTF = referencePriceTF;
    ...
    // console.log(this.props.result.proAllData);
    this.setState({
      proAllData:this.props.result.proAllData,
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        for (let i = 0; i < texturesList.length; i++) {
          if (!values[`salePrice_${i}`] || !values[`sellPrice_${i}`]) {
            message.error('批发价和销售价还有未填，请重新填写再重新提交！');
            return;
          }
        ...
        <Form layout="inline" onSubmit={this.handleSubmit}>
          <div className={styles.modalDiv}>
            <Row>
              ...
              <Col span={24}>
                <div className={styles.contant}>
                  {self.props.result.proAllData.designSource == 2 ?
                    <div className={styles.tabTwo}>
                      <Row>
                        <span>
                          <Col span={2} className={styles.leftTxt}>设计师：</Col>
                          <Col span={21} className={styles.rightTxt}>{self.props.result.proAllData.designerListFrom.designerName}</Col>
                        </span>
                        {self.props.result.proAllData.seriecs ?
                          <span>
                            <Col span={2} className={styles.leftTxt}>产品系列：</Col>
                            <Col span={21} className={styles.rightTxt}>{self.props.result.proAllData.seriecs.seriecsName}</Col>
                            {self.props.result.proAllData.seriecs.seriecsDescription ?
                              <span>
                                <Col span={2} className={styles.leftTxt}>系列描述：</Col>
                                <Col span={22} className={styles.rightTxt}>{self.props.result.proAllData.seriecs.seriecsDescription}</Col>
                              </span>
                                : ''
                              }
                          </span>
                  ...
                  <div className={styles.tabOne}>
                    <Table dataSource={self.props.result.proAllData.textures} {...tableState}>
                      ...
                      <Column
                        ...
                        key="textureWeight"
                            />
                      {self.props.result.proAllData.referencePriceTF?
                      <Column
                        title="预估成本价/￥"
                      ...
                      <Column
                        ...
                        render={(text, record, ind) => (
                          <span>
                            {getFieldDecorator(`sellPrice_${ind}`, {
