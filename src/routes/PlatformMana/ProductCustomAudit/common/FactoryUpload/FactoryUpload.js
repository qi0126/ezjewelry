...
class FactoryUpload extends React.Component {
  ...
  componentDidMount() {
    ...
    // console.log(self.props.result.proAllData.imgList);
      ...
      if (!err) {
        ...
        if (this.props.result.proAllData.classifyName == '对戒') {
          ...
          this.props.result.proAllData.skuPages.forEach((ielem, ind) => {
            ...
            const tempObj = {
              skuId: ielem.skuId,
              wholesalePrice_: values[`wholesalePrice_${ind}_${ielem.skuProps[0].propValue}`] ? values[`wholesalePrice_${ind}_${ielem.skuProps[0].propValue}`] : 0,
            };
            tempSubmitList.push(tempObj);
          });
        } else {
          for (let i = 0; i < texturesList.length; i++) {
            if (!values[`wholesalePrice_${i}`]) {
              message.error('平台批发价还有未填，请重新填写再重新提交！');
              break;
            }
            const tempTextureList = {
              skuId: texturesList[i].skuId,
              wholesalePrice: values[`wholesalePrice_${i}`],
            };
              ...
              <Col span={24}>
                <div className={styles.contant}>
                  ...
                  <div className={styles.tabTwo}>
                    <Row>
                      <Col span={24} className={styles.leftTxt}>供应商名称：<span className={styles.rightTxt}>{self.props.result.proAllData.nickName}</span></Col>
                              ...
                              <TabPane tab={mdata.skuProps[0].propValue} key={ind}>
                                <Row className={styles.tabOneCol}>
                                  ...
                                  <Col span={6} style={{ height: 40, float: 'right' }}>
                                    <span style={{ color: '#333333', marginRight: 10, fontSize: '12px' }}>平台批发价/￥</span>
                                    {getFieldDecorator(`wholesalePrice_${ind}_${mdata.skuProps[0].propValue}`, {
